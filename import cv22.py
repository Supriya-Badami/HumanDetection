import cv2
import imutils
import winsound  # Use this for Windows
...
def start_detection():
    ...
    while True:
        ...
        # Play a sound if any face is detected
        if face_count > 0:
        ...
        key = cv2.waitKey(1)
        if key != -1:  # Any key to exit
            break

    # Release resources
    cv2.destroyAllWindows()
    cap.release()
    login_window.deiconify()  # Show the login window again after exiting
...
def login():
    ...
    if username == "admin" and password == "pass":  # Simple login check
        show_success_window(username)
    else:
        messagebox.showerror("Login Failed", "Invalid username or password.")

# Function to display a bigger "Login Successful" window
def show_success_window(username):
    ...
    success_window.geometry("400x400")  # Set the size of the success window
    success_window.configure(bg='#DDA0DD')  # Background color
    
    # Display the success message
    success_label = tk.Label(success_window, text=f"Welcome, {username}!", font=("Arial", 30), bg='#7B68EE')
...
bg_image = Image.open(r"C:\Project\humanDetect\3.jpg")  # Prefix with r to make it a raw string  # Replace with your image path
